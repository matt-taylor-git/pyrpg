...
def handle_player_turn(hero, enemy, action):
    """Handles the player's turn of combat."""
    events = []
    combat_over = False
    ...
    if action == "attack":
    ...
    elif isinstance(action, tuple) and action[0] == "use_skill":
        skill = action[1]
        events.append({'type': 'use_skill', 'skill_name': skill.name})
        damage = skill.damage + (hero.intelligence // 2 if skill.skill_type == 'magic' else hero.strength // 2)
        ...
        pass  # Handled in the UI, no event needed here, but enemy still gets a turn
    ...
    if not enemy.is_alive():
        ...
        if random.random() < (0.2 + (enemy.level * 0.05)):
            item = generate_random_item(enemy.level)
            hero.add_item(item)
...
def handle_enemy_turn(hero, enemy):
    """Handles the enemy's turn of combat."""
    events = []
    combat_over = False

    if random.random() < 0.3 and enemy.special_attacks:
        damage_type, damage = enemy.perform_special_attack()
        actual_damage, hit_type = hero.take_damage(damage, damage_type)
        if hit_type == "dodged":
            events.append({'type': 'enemy_miss', 'enemy_name': enemy.name, 'attack_type': damage_type})
        else:
            events.append({'type': 'enemy_damage', 'damage': actual_damage, 'enemy_name': enemy.name, 'attack_type': f"special {damage_type}"})
    else:
        actual_damage, hit_type = hero.take_damage(enemy.attack, 'physical')
        if hit_type == "dodged":
